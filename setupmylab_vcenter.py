...
## Functions
def update_session_setting(setting_name, setting_value):
 
    url = "https://%s/api/appliance-settings" % (HOST)
 
    jbody = {}
    jbody["applianceSettings"] = {}
    jbody["applianceSettings"][setting_name] = setting_value
    body = json.dumps(jbody)
 
    response = requests.put(url, headers=HTTP_HEADERS, data=body, verify=False)
    if not response.ok:
        print("Error adding session timeout setting: Response code %s: %s" % (response.status_code, response.text))
        raise Exception("Request error occured")
    data = response.json()
    print("Session Timeout Settings %s updated to %s" % (setting_name, setting_value))
...
def update_monitoring_setting(setting_name, setting_value):
 
    url = "https://%s/api/monitoring-settings" % (HOST)
 
    jbody = {}
    jbody["monitoringSettings"] = {}
    jbody["monitoringSettings"][setting_name] = setting_value
    body = json.dumps(jbody)
 
    response = requests.put(url, headers=HTTP_HEADERS, data=body, verify=False)
    if not response.ok:
        print("Error adding monitoring settings: Response code %s: %s" % (response.status_code, response.text))
        raise Exception("Request error occured")
    data = response.json()
    print("Monitoring Settings %s updated to %s" % (setting_name, setting_value))
...
## Main
## Appliance Settings
update_session_setting("userBrowserSessionTimeout", "60")
...
## Monitoring Settings
update_monitoring_setting("autoManageChecks", True)
...
add_instance_logo(instance_type_id, "/ubuntu_logo.svg")
...
## Module 3 - Create Custom DB Instance
create_instance_type("Custom DB", "customDb")
instance_type_id = get_instance_type_id_name("Custom DB")
create_layout("Custom DB Layout", instance_type_id, ubuntu_node_id, "")
 
## Module 3 - Create Custom App Instance
create_node_type("Custom App node","custAppnode","Latest",ubuntuvi)
ubuntu_node_id = get_node_type_id_name("Custom App node")
create_instance_type("Custom App", "customApp")
instance_type_id = get_instance_type_id_name("Custom App")
